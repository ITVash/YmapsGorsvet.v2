...
const TitleBox = ({ className, title, area, edit, column, base, editBase, coup }) => {
  base && console.log('base', base);
  return (
    <div className={classNames(`${className}__title`)}>
      {edit ? (
        <>
          <label htmlFor="">{className === 'info-box' ? 'Название шкафа' : 'Название опоры'}</label>
          <Input value={ title[0] } onChange={e => editBase({...base, [column[0]] : e.target.value})} style={{width: '90%'}} />
          <span>
            {className === 'info-opora' && (
              <select name="areasID" className="opt"
                onChange={e => editBase({...base, [column[2]] : e.target.value})}
              > 
                <option value="0">Шкаф Управления</option>
                {coup.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}
              </select>
            )}
            <select name="areasID" className="opt"
              onChange={e => editBase({...base, [column[1]] : e.target.value})}
            >
